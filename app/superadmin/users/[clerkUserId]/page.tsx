      ...
      <header className="border-b border-gray-800/60 px-4 sm:px-8 py-4 flex items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <Link href="/superadmin/users" className="text-gray-500 hover:text-white text-sm">← Users</Link>
            <span className="text-gray-700">/</span>
            <h1 className="text-lg font-semibold">{profile?.name || <span className="text-gray-500 italic">No name</span>}</h1>
          </div>
          <p className="text-gray-500 text-xs mt-0.5 font-mono">{clerkUserId}</p>
        </div>
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          <span>·</span>
          <span className="text-emerald-400 font-semibold">${paidTotal.toLocaleString()} paid</span>
        </div>
      </header>

      {/* Tabs */}
      <div className="border-b border-gray-800 px-4 sm:px-8">
      ...
      <div className="flex-1 overflow-auto px-4 sm:px-8 py-6 max-w-4xl">
            ...
            {[
              ...
              <div key={f.label}>
                ...
                <input
                  ...
                  className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-emerald-500"
                />
              </div>
            ))}
            {saveMsg && (
              <p className={`text-sm px-3 py-2 rounded-lg ${saveMsg.ok ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}>
                {saveMsg.text}
              </p>
            )}
            <button
        ...
        {tab === "bids" && (
          <div className="space-y-2">
            {bids.length === 0 ? (
              <p className="text-gray-500 text-sm py-8 text-center">No bids placed.</p>
            ) : (
              bids.map((bid) => (
                <div key={bid.id} className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 flex items-center gap-4">
                  {bid.item?.photos[0] ? (
                  ...
                  <div className="text-right shrink-0 flex items-center gap-3">
                    <div>
                      <div className="text-emerald-400 font-semibold text-sm">${bid.amount.toLocaleString()}</div>
                      <div className="text-gray-600 text-xs">{new Date(bid.placedAt).toLocaleDateString()}</div>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[bid.status] || "bg-gray-700 text-gray-400"}`}>
                      {bid.status}
                    </span>
        ...
        {tab === "payments" && (
          <div className="space-y-2">
            {payments.length === 0 ? (
              <p className="text-gray-500 text-sm py-8 text-center">No payment records.</p>
            ) : (
              payments.map((p) => (
                <div key={p.id} className="bg-gray-900 border border-gray-800 rounded-xl px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-gray-500 text-xs mt-0.5">{p.item?.auction?.organization.name} · {new Date(p.createdAt).toLocaleDateString()}</div>
                    ...
                    <div className="text-right shrink-0 flex items-center gap-3">
                      <div>
                        ...
                        {p.taxAmount ? <div className="text-gray-500 text-xs">+${p.taxAmount} tax</div> : null}
                      </div>
                      ...
                      <select
                        ...
                        className="bg-gray-800 border border-gray-700 rounded-lg px-2 py-1 text-xs text-white focus:outline-none disabled:opacity-50"
                      >
                  ...
                  {p.stripePaymentIntentId && (
                    <div className="text-gray-700 text-xs mt-2 font-mono truncate">{p.stripePaymentIntentId}</div>
                  )}
                </div>
              ))
            )}
        ...
        {tab === "orgs" && (
          <div className="space-y-2">
            {memberships.length === 0 ? (
              <p className="text-gray-500 text-sm py-8 text-center">Not a member of any organization.</p>
            ) : (
              memberships.map((m) => (
                <div key={m.id} className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-4 flex items-center justify-between">
                  <div>
                    <Link href={`/superadmin/orgs/${m.organization.id}`} className="font-medium hover:text-emerald-400 transition-colors">
                      {m.organization.name}
                    </Link>
                    <div className="text-gray-500 text-xs mt-0.5">/{m.organization.slug}</div>
                  </div>
                  <span className="text-xs bg-gray-800 text-gray-300 px-3 py-1 rounded-full font-medium">{m.role}</span>
                </div>
              ))
            )}
          </div>
