    ...
    return (
      <main className="min-h-screen bg-gray-950 text-white">
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-16 text-center">
          ...
          <p className="text-gray-600 text-sm mt-5">Type at least 2 characters to search</p>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        <p className="text-gray-500 text-sm mb-8">
          {total === 0
            ? <>No results for <span className="text-white font-semibold">&ldquo;{query}&rdquo;</span></>
            : <>{total} result{total !== 1 ? "s" : ""} for <span className="text-white font-semibold">&ldquo;{query}&rdquo;</span></>
          }
        </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {items.map(item => (
                <Link
                  ...
                  className="bg-gray-900 border border-gray-800 hover:border-emerald-500/40 rounded-2xl p-4 flex items-center gap-4 transition-all hover:shadow-[0_0_20px_rgba(52,211,153,0.05)] group"
                >
                  {item.photos[0] ? (
                    <img
                    ...
                    <div className="w-14 h-14 bg-gray-800 rounded-xl shrink-0 flex items-center justify-center text-gray-600">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold truncate group-hover:text-emerald-400 transition-colors text-sm">
                      {item.title}
                    </div>
              ...
              {auctions.map(auction => {
                ...
                return (
                  <Link
                    ...
                    className="bg-gray-900 border border-gray-800 hover:border-emerald-500/40 rounded-2xl p-4 flex items-center justify-between gap-4 transition-all hover:shadow-[0_0_20px_rgba(52,211,153,0.05)] group"
                  >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {orgs.map(org => (
                <Link
                  key={org.id}
                  href={`/${org.slug}`}
                  className="bg-gray-900 border border-gray-800 hover:border-gray-700 rounded-2xl p-4 transition-all group"
                >
                  <div className="mb-3">
                    <OrgLogo name={org.name} logoUrl={org.logoUrl} size="sm" />
                  </div>
                  <div className="font-semibold text-sm truncate group-hover:text-emerald-400 transition-colors">
                    {org.name}
                  </div>
