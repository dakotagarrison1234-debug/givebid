...
function NavLink({
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-[#f2efe8] transition-colors text-sm group ${className}`}
    >
      <span className="w-5 flex items-center justify-center shrink-0 text-[#8c8778] group-hover:text-[#4a4640]">{iconEl}</span>
      <span className="flex-1 min-w-0">
        <span className="block text-[#2c2a24] group-hover:text-[#1a1916]">{label}</span>
        {sublabel && <span className="block text-xs text-[#b0a99a] truncate mt-0.5">{sublabel}</span>}
      </span>
    </Link>
  );
}
...
export default function UserMenu() {
  ...
  const drawer = (
    <>
      {/* Full-screen backdrop — keep dark for contrast */}
      <div
      ...
      >
        {/* User info header */}
        <div style={{ padding: "20px", borderBottom: "1px solid #e5e0d5", display: "flex", alignItems: "flex-start", gap: "12px", justifyContent: "space-between", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ color: "#1a1916", fontWeight: 600, fontSize: 14, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{displayName}</p>
              <p style={{ color: "#8c8778", fontSize: 12, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{email}</p>
              {me?.orgName && (
                <p style={{ color: "#09a7ad", fontSize: 12, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginTop: 2 }}>
                  {me.orgName}
                  {me.role && <span style={{ color: "#b0a99a" }}> · {me.role.toLowerCase()}</span>}
                </p>
              )}
            </div>
          </div>
          <button
            onClick={() => setOpen(false)}
            style={{ color: "#b0a99a", background: "none", border: "none", cursor: "pointer", padding: 0, flexShrink: 0 }}
        ...
        <nav style={{ flex: 1, padding: "12px", overflowY: "auto", minHeight: 0 }}>
          ...
          {me?.isSuperAdmin && (
            <>
              <div style={{ borderTop: "1px solid #e5e0d5", margin: "8px 0" }} />
              <NavLink
                href="/superadmin"
                iconEl={<IcoBolt />}
                label="Super Admin"
        ...
        {/* Sign out */}
        <div style={{ padding: "12px 12px 16px", borderTop: "1px solid #e5e0d5", flexShrink: 0 }}>
          <button
            onClick={async () => {
              setOpen(false);
              await signOut();
            ...
            style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", borderRadius: 12, color: "#dc2626", background: "none", border: "none", cursor: "pointer", fontSize: 14 }}
          >
